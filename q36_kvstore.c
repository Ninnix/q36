 ...
 * q36-server still owns the automatic byte-prefix cache policy built on top of
 * this file; q36-agent uses only the same durable format for explicit sessions,
 * with its own policy in q36_agent.c.  Protocol-specific extras, such as the
 * server's tool-id -> exact Qwen tool-call trailer, are attached through trailer hooks and
 ...
 * q36_session internals become unsafe to restore across runtime changes. */
...
static const char *kv_log_name(const q36_kvstore *kc) {
    return kc && kc->log_name ? kc->log_name : "q36";
}

static void kv_logf(q36_kvstore *kc, q36_kvstore_log_type type,
                    const char *fmt, ...) {
    if (!kc || !kc->log) return;
    va_list ap;
    va_start(ap, fmt);
...
uint8_t q36_kvstore_reason_code(const char *reason) {
    if (!reason) return Q36_KVSTORE_REASON_UNKNOWN;
    if (!strcmp(reason, "cold")) return Q36_KVSTORE_REASON_COLD;
    if (!strcmp(reason, "continued")) return Q36_KVSTORE_REASON_CONTINUED;
    if (!strcmp(reason, "evict")) return Q36_KVSTORE_REASON_EVICT;
    if (!strcmp(reason, "shutdown")) return Q36_KVSTORE_REASON_SHUTDOWN;
    if (!strcmp(reason, "agent-system")) return Q36_KVSTORE_REASON_AGENT_SYSTEM;
    if (!strcmp(reason, "agent-session")) return Q36_KVSTORE_REASON_AGENT_SESSION;
    return Q36_KVSTORE_REASON_UNKNOWN;
}

const char *q36_kvstore_key_kind(uint8_t ext_flags) {
    if (ext_flags & Q36_KVSTORE_EXT_RESPONSES_VISIBLE) return "responses-visible";
    if (ext_flags & Q36_KVSTORE_EXT_THINKING_VISIBLE) return "thinking-visible";
    return "token-text";
}
...
char *q36_kvstore_path_for_sha(q36_kvstore *kc, const char sha[41]) {
    ...
    memcpy(name, sha, 40);
    memcpy(name + 40, ".kv", 4);
    return q36_kvstore_path_join(kc->dir, name);
}
...
void q36_kvstore_clear(q36_kvstore *kc) {
    for (int i = 0; i < kc->len; i++) q36_kvstore_entry_free(&kc->entry[i]);
    free(kc->entry);
    kc->entry = NULL;
    kc->len = 0;
    kc->cap = 0;
...
static void kv_cache_push(q36_kvstore *kc, q36_kvstore_entry e) {
    if (kc->len == kc->cap) {
...
void q36_kvstore_fill_header(uint8_t h[Q36_KVSTORE_FIXED_HEADER],
    ...
    memset(h, 0, Q36_KVSTORE_FIXED_HEADER);
    h[0] = KV_CACHE_MAGIC0;
    h[1] = KV_CACHE_MAGIC1;
    h[2] = KV_CACHE_MAGIC2;
    ...
    h[6] = ext_flags;
    h[7] = model_id;
    q36_kvstore_le_put32(h + 8, tokens);
    q36_kvstore_le_put32(h + 12, hits);
    q36_kvstore_le_put32(h + 16, ctx_size);
...
bool q36_kvstore_read_header(FILE *fp, q36_kvstore_entry *e,
    ...
    e->quant_bits = h[4];
    e->reason = h[5] <= Q36_KVSTORE_REASON_AGENT_SESSION ? h[5] :
                Q36_KVSTORE_REASON_UNKNOWN;
    e->ext_flags = h[6];
    e->model_id = h[7];
    e->tokens = q36_kvstore_le_get32(h + 8);
    e->hits = q36_kvstore_le_get32(h + 12);
    e->ctx_size = q36_kvstore_le_get32(h + 16);
...
bool q36_kvstore_read_entry_file(const char *path, const char sha[41],
    ...
    struct stat st;
    if (stat(path, &st) != 0 ||
        st.st_size < (off_t)(Q36_KVSTORE_FIXED_HEADER + 4))
        return false;
    FILE *fp = fopen(path, "rb");
    if (!fp) return false;
    q36_kvstore_entry e = {0};
    uint32_t text_bytes = 0;
    bool ok = q36_kvstore_read_header(fp, &e, &text_bytes);
    fclose(fp);
    if (!ok) return false;
    const uint64_t fixed = Q36_KVSTORE_FIXED_HEADER + 4ull;
...
static void kv_cache_refresh(q36_kvstore *kc) {
    if (!kc->enabled) return;
    q36_kvstore_clear(kc);
    ...
    while ((de = readdir(d)) != NULL) {
        ...
        char *path = q36_kvstore_path_join(kc->dir, de->d_name);
        q36_kvstore_entry e = {0};
        if (q36_kvstore_read_entry_file(path, sha, &e)) kv_cache_push(kc, e);
        free(path);
    }
    closedir(d);
}
...
bool q36_kvstore_touch_file(const char *path, uint32_t hits) {
    ...
    if (ok) {
        ...
        q36_kvstore_fill_header(h, e.model_id, e.quant_bits, e.reason, e.ext_flags,
...
static bool kv_cache_incoming_supersedes_continued(
    ...
    char prefix_sha[41];
    q36_kvstore_sha1_bytes_hex(incoming->text, (size_t)e->text_bytes,
                               prefix_sha);
    return !strcmp(prefix_sha, e->sha);
}

static bool kv_cache_reason_is_anchor(uint8_t reason) {
    return reason == Q36_KVSTORE_REASON_COLD ||
           reason == Q36_KVSTORE_REASON_EVICT ||
           reason == Q36_KVSTORE_REASON_SHUTDOWN;
}

double q36_kvstore_entry_eviction_score(
        const q36_kvstore_entry *e,
    ...
    if (used_at == 0) {
        ...
        effective_hits *= exp2(-elapsed / (double)Q36_KVSTORE_HIT_HALF_LIFE_SECONDS);
...
void q36_kvstore_evict(q36_kvstore *kc, const q36_tokens *live,
    ...
    while (total > target && kc->len > 0) {
        int victim = 0;
        double victim_score =
            q36_kvstore_entry_eviction_score(&kc->entry[0], live, now,
                                             incoming);
        for (int i = 1; i < kc->len; i++) {
            double score =
                q36_kvstore_entry_eviction_score(&kc->entry[i], live, now,
                                                 incoming);
        ...
        q36_kvstore_entry e = kc->entry[victim];
        if (unlink(e.path) == 0) {
            kv_logf(kc, Q36_KVSTORE_LOG_KVCACHE,
...
bool q36_kvstore_open(q36_kvstore *kc, const char *dir, uint64_t budget_mb,
    ...
    kc->log_ud = log_ud;
    if (!kv_mkdir_p(dir)) {
        kv_logf(kc, Q36_KVSTORE_LOG_DEFAULT,
    ...
    kv_logf(kc, Q36_KVSTORE_LOG_KVCACHE,
            ...
            (unsigned long long)Q36_KVSTORE_HIT_HALF_LIFE_SECONDS);
    return true;
}
...
void q36_kvstore_build_prompt_from_exact_prefix_and_text_suffix(
    ...
    q36_tokens suffix = {0};
    /* The suffix may start with Q36 chat markers such as <|im_start|> or
...
static int kv_cache_continued_step(const q36_kvstore *kc) {
...
int q36_kvstore_suppress_continued_store(q36_kvstore *kc, int tokens) {
    if (q36_kvstore_continued_store_target(kc, tokens) != tokens) return -1;
    int old = kc->continued_last_store_tokens;
    q36_kvstore_note_store(kc, tokens);
    return old;
}
...
static bool kv_cache_file_text_matches(const char *path, const char sha[41],
    ...
    char stored_sha[41];
    q36_kvstore_sha1_bytes_hex(stored, text_bytes, stored_sha);
...
static bool kv_cache_existing_compatible(q36_kvstore *kc, const char *path,
                                         const char sha[41],
                                         const char *text, size_t text_len,
                                         int model_id, int quant_bits, int ctx_size) {
    if (access(path, F_OK) != 0) return false;
    q36_kvstore_entry e = {0};
    if (!q36_kvstore_read_entry_file(path, sha, &e)) return false;
...
static bool kv_trailer_serialized_size(const q36_kvstore_trailer_hooks *hooks,
                                       const char *text,
                                       uint64_t *bytes_out) {
...
static bool kv_trailer_write(const q36_kvstore_trailer_hooks *hooks,
...
static void kv_cache_rewrite_trailer(q36_kvstore *kc, const char *path,
                                     const char *text,
    ...
    uint32_t text_bytes = 0;
    bool ok = q36_kvstore_read_header(fp, &hdr, &text_bytes);
    uint64_t end = Q36_KVSTORE_FIXED_HEADER + 4ull +
        ...
        ftruncate(fileno(fp), (off_t)end) == 0)
    {
        ...
        if (ok && ignored > 0) {
            uint8_t h[Q36_KVSTORE_FIXED_HEADER];
            uint64_t now = (uint64_t)time(NULL);
            q36_kvstore_fill_header(h, hdr.model_id, hdr.quant_bits, hdr.reason,
...
bool q36_kvstore_store_live_prefix_text(q36_kvstore *kc,
    ...
    char save_err[160] = {0};
    const q36_tokens *live_tokens = q36_session_tokens(session);
    if (!live_tokens ||
    ...
    if (text_override) {
        ...
        text = q36_kvstore_render_tokens_text(engine, &store_tokens, &text_len);
    }
    ...
    char *path = q36_kvstore_path_for_sha(kc, sha);
    const uint8_t reason_code = q36_kvstore_reason_code(reason);

    if (kv_cache_existing_compatible(kc, path, sha, text, text_len,
                                     model_id,
                                     quant_bits, q36_session_ctx(session))) {
    ...
    if (q36_session_stage_payload(session, &staged,
    ...
    if (!q36_kvstore_file_size_fits(kc, (uint64_t)text_len, payload_bytes,
                                    trailer_est_bytes,
    ...
    q36_kvstore_eviction_context incoming = {
        .text = text,
        .text_len = text_len,
        .model_id = (uint8_t)model_id,
        .quant_bits = (uint8_t)quant_bits,
        .ctx_size = (uint32_t)q36_session_ctx(session),
        .reject_different_quant = kc->reject_different_quant,
    };
    q36_kvstore_evict(kc, live_tokens, est_file_bytes, &incoming);
    ...
    q36_kvstore_fill_header(h, (uint8_t)model_id, (uint8_t)quant_bits,
                            ...
                            (uint32_t)q36_session_ctx(session),
    ...
    if (ok && !q36_kvstore_file_size_fits(kc, (uint64_t)text_len, payload_bytes,
                                          trailer_bytes,
                                          &final_file_bytes,
    ...
    if (!ok) {
        ...
        kv_logf(kc, Q36_KVSTORE_LOG_KVCACHE,
                ...
                (double)(Q36_KVSTORE_FIXED_HEADER + 4ull + text_len + payload_bytes + trailer_bytes) / (1024.0 * 1024.0),
                save_ms);
    }
    q36_session_payload_file_free(&staged);
    free(tmp);
...
bool q36_kvstore_store_live_prefix(q36_kvstore *kc,
                                   ...
                                   char *err,
                                   size_t err_len) {
    return q36_kvstore_store_live_prefix_text(kc, engine, session, tokens,
...
bool q36_kvstore_maybe_store_continued(q36_kvstore *kc,
                                       ...
                                       char *err,
                                       size_t err_len) {
    const q36_tokens *tokens = q36_session_tokens(session);
    if (!tokens) return false;
    const int target = q36_kvstore_continued_store_target(kc, tokens->len);
    if (target == 0) return false;
    if (q36_kvstore_store_live_prefix(kc, engine, session, tokens, target,
                                      "continued", hooks, err, err_len))
    {
        q36_kvstore_note_store(kc, target);
        return true;
    }
    return false;
}
...
int q36_kvstore_find_text_prefix(q36_kvstore *kc, const char *prompt_text,
    ...
    for (int i = 0; i < kc->len; i++) {
        q36_kvstore_entry *e = &kc->entry[i];
        ...
        if (best >= 0) {
            q36_kvstore_entry *b = &kc->entry[best];
            if (e->text_bytes < b->text_bytes) continue;
            if (e->text_bytes == b->text_bytes && e->tokens <= b->tokens) continue;
        }
        char sha[41];
        q36_kvstore_sha1_bytes_hex(prompt_text, (size_t)e->text_bytes, sha);
...
int q36_kvstore_try_load_text(q36_kvstore *kc,
    ...
    int idx = q36_kvstore_find_text_prefix(kc, prompt_text, model_id, quant_bits,
                                           q36_session_ctx(session));
    if (idx < 0) return 0;

    q36_kvstore_entry e = kc->entry[idx];
    ...
    bool header_ok = q36_kvstore_read_header(fp, &hdr, &text_bytes);
    ...
    if (header_ok) {
        if (hdr.model_id != (uint8_t)model_id) {
            ...
            if (fread(cached_text, 1, text_bytes, fp) != text_bytes) {
                ...
                char text_sha[41];
                q36_kvstore_sha1_bytes_hex(cached_text, text_bytes, text_sha);
                if (strcmp(text_sha, e.sha)) {
                    header_ok = false;
                    fail_reason = "cached text hash mismatch";
                } else if (!q36_kvstore_byte_prefix_match(prompt_text, prompt_bytes,
    ...
    int loaded = 0;
    if (header_ok &&
        q36_session_load_payload(session, fp, hdr.payload_bytes, err, sizeof(err)) == 0)
    {
        const q36_tokens *loaded_tokens = q36_session_tokens(session);
        if (loaded_tokens && loaded_tokens->len == (int)hdr.tokens) {
            ...
            if (effective_prompt) {
                ...
                q36_kvstore_build_prompt_from_exact_prefix_and_text_suffix(
            ...
            q36_session_invalidate(session);
            unlink(path);
            kv_logf(kc, Q36_KVSTORE_LOG_KVCACHE,
        ...
        if (header_ok) q36_session_invalidate(session);
        kv_logf(kc, Q36_KVSTORE_LOG_KVCACHE,
    ...
    if (loaded > 0) {
        ...
        const char *key_kind = q36_kvstore_key_kind(hdr.ext_flags);
        q36_kvstore_touch_file(path, hdr.hits + 1);
        kv_logf(kc, Q36_KVSTORE_LOG_KVCACHE,
