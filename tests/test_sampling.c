...
static int argmax(const float *logits, uint32_t n) {
    int best = 0;
...
/* Q36's sampler before the min-p fast path. */
static int reference_sample(const float *logits, uint32_t n_vocab,
                            float temperature, int top_k,
                            float top_p, float min_p, uint64_t *rng) {
    if (temperature <= 0.0f) return argmax(logits, n_vocab);
    ...
    if (top_k > 0) {
        ...
        if (n == 0) return argmax(logits, n_vocab);
        ...
        float r = rng_f32(rng) * filtered_sum;
    ...
    if (finite == 0) return argmax(logits, n_vocab);
    ...
    candidate *cand = malloc((size_t)finite * sizeof(*cand));
    ...
    qsort(cand, n, sizeof(*cand), candidate_cmp);
    const float min_prob = (cand[0].prob / sum) *
                           (min_p > 0.0f ? min_p : 0.0f);
    float filtered_sum = 0.0f;
    uint32_t filtered = 0;
    for (uint32_t i = 0; i < n; i++) {
...
static void compare_case(const float *logits, float *scratch, uint32_t n,
    ...
    for (uint64_t seed = 0; seed < 256; seed++) {
        ...
        const int ref = reference_sample(logits, n, temperature, top_k,
                                         top_p, min_p, &ref_rng);
        const int opt = q36_test_sample_logits(logits, n, temperature, top_k,
...
static void benchmark(const float *logits, float *scratch, uint32_t n,
                      float top_p, const char *label) {
    ...
    for (int i = 0; i < iterations; i++) {
        checksum += (uint64_t)reference_sample(logits, n, 1.0f, 0,
                                               top_p, 0.05f, &ref_rng);
    }
    ...
    start = now_sec();
    for (int i = 0; i < iterations; i++) {
        checksum += (uint64_t)q36_test_sample_logits(
            logits, n, 1.0f, 0, top_p, 0.05f, &opt_rng, scratch);
    }
    const double optimized_ms = (now_sec() - start) * 1000.0;

    CHECK(ref_rng == opt_rng, "%s RNG state mismatch", label);
    printf("%s: reference %.3f ms, optimized %.3f ms, %.2fx, checksum=%llu\n",
           label, reference_ms, optimized_ms,
...
int main(void) {
    ...
    float boundary_scratch[
        sizeof(boundary_logits) / sizeof(boundary_logits[0])];
    compare_case(boundary_logits, boundary_scratch,
                 (uint32_t)(sizeof(boundary_logits) /
                            sizeof(boundary_logits[0])),
    ...
    const int null_opt = q36_test_sample_logits(
        boundary_logits,
        (uint32_t)(sizeof(boundary_logits) / sizeof(boundary_logits[0])),
    ...
    float nonfinite_scratch[
        sizeof(nonfinite_logits) / sizeof(nonfinite_logits[0])];
    ...
    benchmark(logits, scratch, perf_n, 1.0f, "sampling default min-p");
    benchmark(logits, scratch, perf_n, 0.9f, "sampling top-p + min-p");
