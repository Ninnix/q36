...
typedef decltype(kernel_mul_mm_id<32, half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, block_q2_K, QK_NL, dequantize_q2_K, half, half4x4, half, half2x4, true>) mul_mm_id_compact_f16_rhs;
...
template [[host_name("kernel_mul_mm_id_q2_K_f16_compact")]]
kernel mul_mm_id_compact_f16_rhs kernel_mul_mm_id<32, half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, block_q2_K, QK_NL, dequantize_q2_K, half, half4x4, half, half2x4, true>;
...
// Routed-expert grouped matmul on the Metal 4 TensorOps/MPP pipeline. Q36's
// compact map stores (expert, first-row) pairs after the expert-major ids, so
// the kernel can use the existing fixed-size dispatch without another work
// buffer. The barrier after mm.run keeps staged tiles alive until the
// cooperative matmul has finished reading them.
template<typename S0, typename S0_4x4, typename S0_8x8,
...
kernel void kernel_mul_mm_id_mpp(
    ...
    device const int32_t *ids_i32 = (device const int32_t *)hids;
    device const int32_t *blocks = ids_i32 + args.ne02*args.ne21;
    const int im = blocks[2u*tgpig.z];
    if (im < 0) return;
    const int r0 = tgpig.y*NR0;
    const int r1 = blocks[2u*tgpig.z + 1u];
    ...
    device const block_q *x =
        (device const block_q *)(src0 + args.nb01*(r0 + lr0) + offset0) +
        offset1;
    ...
    auto cT = mm.template
        get_destination_cooperative_tensor<decltype(tB), decltype(tA), float>();
...
typedef decltype(kernel_mul_mm_id_mpp<
    half, half4x4, simdgroup_half8x8,
    half, half2x4, simdgroup_half8x8,
    block_q2_K, QK_NL, dequantize_q2_K,
    float, float4x4, float, float2x4>) mul_mm_id_mpp_f32_t;
typedef decltype(kernel_mul_mm_id_mpp<
    half, half4x4, simdgroup_half8x8,
    half, half2x4, simdgroup_half8x8,
    block_q2_K, QK_NL, dequantize_q2_K,
    half, half4x4, half, half2x4>) mul_mm_id_mpp_f16_rhs_t;

template [[host_name("kernel_mul_mm_id_iq2_xxs_f32_mpp")]]
kernel mul_mm_id_mpp_f32_t kernel_mul_mm_id_mpp<
    half, half4x4, simdgroup_half8x8,
    half, half2x4, simdgroup_half8x8,
    block_iq2_xxs, QK_NL, dequantize_iq2_xxs,
    float, float4x4, float, float2x4>;
template [[host_name("kernel_mul_mm_id_q2_K_f16_mpp")]]
kernel mul_mm_id_mpp_f16_rhs_t kernel_mul_mm_id_mpp<
    half, half4x4, simdgroup_half8x8,
    half, half2x4, simdgroup_half8x8,
    block_q2_K, QK_NL, dequantize_q2_K,
