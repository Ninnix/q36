...
static q36_think_mode think_mode_from_enabled(bool enabled, q36_think_mode effort) {
    if (!enabled) return Q36_THINK_NONE;
...
static bool parse_anthropic_content_block(const char **p, const char *role, chat_msg *msg) {
    ...
    if (type && !strcmp(type, "tool_use") && !strcmp(role, "assistant")) {
        ...
        append_xml_text_escaped(&b, tool_result);
        buf_puts(&b, "\n</tool_response>");
        free(msg->content);
...
static void append_tools_prompt_text(buf *b, const char *tool_schemas) {
    ...
    buf_puts(b,
        ...
        "</function>\n"
        "</tool_call>\n\n"
        "Required parameters MUST be specified. You may write reasoning before the tool call, but not after it.");
}
...
static char *render_chat_prompt_text(const chat_msgs *msgs, const char *tool_schemas,
    ...
    for (int i = 0; i < msgs->len; i++) {
        ...
        if (role_is_system(m->role)) {
            ...
            if (!pending_tool_result) buf_puts(&out, "<|im_start|>user\n");
            buf_puts(&out, "<tool_response>\n");
            append_xml_text_escaped(&out, m->content);
            buf_puts(&out, "\n</tool_response>");
            buf_puts(&out, "<|im_end|>\n");
            pending_assistant = true;
...
static bool parse_responses_input_item(const char **p, chat_msgs *msgs) {
    ...
    while (**p && **p != '}') {
        ...
        if (!strcmp(key, "type")) {
            ...
            free(output);
            if (!json_content(p, &output)) {
                free(key);
                goto bad;
            }
        } else if (!strcmp(key, "call_id")) {
            free(call.id);
            if (!json_string(p, &call.id)) {
                free(key);
                goto bad;
            }
        } else if (!strcmp(key, "name")) {
            free(call.name);
            if (!json_string(p, &call.name)) {
                free(key);
                goto bad;
            }
        } else if (!strcmp(key, "arguments")) {
            free(call.arguments);
            if (!parse_responses_string_or_json(p, &call.arguments)) {
                free(key);
                goto bad;
            }
        } else if (!json_skip_value(p)) {
    ...
    if (type && !strcmp(type, "reasoning")) goto done;
    ...
    } else if (type && !strcmp(type, "function_call_output")) {
...
static void append_responses_output(buf *b, const char *id, const char *text,
    ...
    if (reasoning && reasoning[0]) {
        buf_puts(b, "{\"id\":\"rs_");
        buf_puts(b, id);
        buf_puts(b, "\",\"type\":\"reasoning\",\"status\":");
        json_escape(b, status);
        buf_puts(b, ",\"summary\":[{\"type\":\"summary_text\",\"text\":");
        json_escape(b, reasoning);
        buf_puts(b, "}]}" );
        output_index++;
    }
    if (text && text[0]) {
        ...
        buf_puts(b, "{\"id\":\"msg_");
        buf_puts(b, id);
        buf_puts(b, "\",\"type\":\"message\",\"status\":");
        json_escape(b, status);
        buf_puts(b, ",\"role\":\"assistant\",\"content\":[{\"type\":\"output_text\",\"text\":");
        json_escape(b, text);
    ...
    for (int i = 0; calls && i < calls->len; i++) {
        ...
        if (output_index++) buf_putc(b, ',');
        buf_puts(b, "{\"id\":\"fc_");
        buf_printf(b, "%s_%d", id, i);
        buf_puts(b, "\",\"type\":\"function_call\",\"status\":");
        json_escape(b, status);
        buf_puts(b, ",\"call_id\":");
...
static bool responses_sse_finish(int fd, const request *r, const char *id,
    ...
    if (ok && reasoning && reasoning[0]) {
        buf_printf(&b,
            "{\"type\":\"response.reasoning_summary_text.delta\",\"item_id\":\"rs_%s\",\"output_index\":%d,\"summary_index\":0,\"delta\":",
            id, output_index);
        json_escape(&b, reasoning);
        buf_putc(&b, '}');
        ok = responses_sse_event(fd, "response.reasoning_summary_text.delta", b.ptr);
        buf_free(&b);
        output_index++;
    }
    if (ok && text && text[0]) {
        buf_printf(&b,
            "{\"type\":\"response.output_text.delta\",\"item_id\":\"msg_%s\",\"output_index\":%d,\"content_index\":0,\"delta\":",
            id, output_index);
        json_escape(&b, text);
        buf_putc(&b, '}');
        ok = responses_sse_event(fd, "response.output_text.delta", b.ptr);
        buf_free(&b);
        output_index++;
    }
 ...
 *
 *   prompt-without-final-<think> + </think> + visible-content + <|im_end|>\n
 *
...
static void generate_job(server *s, server_slot *slot, job *j) {
    ...
    if (server_session_sync(s, slot, prompt_for_sync, err, sizeof(err)) != 0) {
        q36_tokens_free(&effective_prompt);
        q36_session_set_progress(slot->session, NULL, NULL);
        trace_event(s, trace_id, "prefill failed: %s", err);
        http_error(j->fd, 500, err);
        ...
        saw_tool_start && !saw_tool_end && strcmp(finish, "error") != 0)
    {
         ...
         * before closing the QWEN_TOOL block, fail the turn instead of letting clients
...
static void usage(FILE *fp) {
    fprintf(fp,
        ...
        "  --prefill-chunk N\n"
        "      Vulkan graph prefill chunk size. Default: auto.\n"
        "  --quality\n"
        ...
        "  --batched-session N\n"
        "      Keep N resident sessions and batch decode-ready Vulkan requests.\n"
        "  --cors\n"
...
static void test_qwen_tool_prompt_preserves_tool_supplied_text(void) {
    ...
    char *prompt = render_chat_prompt_text(&msgs, "{}", NULL, Q36_THINK_HIGH);
    TEST_ASSERT(prompt != NULL);
    TEST_ASSERT(strstr(prompt, "&lt;tool_call&gt;not a real tool call") != NULL);
    TEST_ASSERT(strstr(prompt, "<tool_response><tool_call>") == NULL);
    free(prompt);
    chat_msgs_free(&msgs);
}
...
static void test_tool_marker_state_ignores_orphan_end(void) {
    ...
    orphan_end = false;
    observe_tool_markers(Q36_TOOL_CALLS_START "\n" Q36_INVOKE_START "bash>",
                         &saw_start, &saw_end, &orphan_end);
    TEST_ASSERT(saw_start);
    TEST_ASSERT(!saw_end);
    TEST_ASSERT(!orphan_end);

    observe_tool_markers(Q36_INVOKE_END "\n" Q36_TOOL_CALLS_END,
                         &saw_start, &saw_end, &orphan_end);
    TEST_ASSERT(saw_start);
    TEST_ASSERT(saw_end);
}
...
static void test_thinking_checkpoint_canonical_matches_future_prompt(void) {
    ...
    char *prompt_text = render_chat_prompt_text(&prefix_msgs, NULL, NULL, Q36_THINK_HIGH);
    ...
    char *future_prompt = render_chat_prompt_text(&history_msgs, NULL, NULL, Q36_THINK_HIGH);
...
static void test_thinking_canonical_empty_content(void) {
    ...
    char *prompt_text = render_chat_prompt_text(&msgs, NULL, NULL, Q36_THINK_HIGH);
    size_t pt_len = strlen(prompt_text);
    ...
    char *future = render_chat_prompt_text(&history, NULL, NULL, Q36_THINK_HIGH);
...
static void test_thinking_canonical_multi_turn(void) {
    ...
    char *prompt_text = render_chat_prompt_text(&turn2_prefix, NULL, NULL, Q36_THINK_HIGH);
    size_t pt_len = strlen(prompt_text);
    TEST_ASSERT(!memcmp(prompt_text + pt_len - 7, "<think>", 7));
    ...
    char *future = render_chat_prompt_text(&future_msgs, NULL, NULL, Q36_THINK_HIGH);
